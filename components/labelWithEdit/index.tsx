...
export default function LabelWithEdit({
  ...
  const editTreeMutation = useMutation({
    ...
    onSuccess: (data) => {
      setTree((oldTree: TreeWithComponents) => {
        return {
          ...oldTree,
          title: data.title,
        }
    ...
    <>
      {
        !enableEdit && (
          <Tooltip text="Click to edit tree name">
            <Label className="group relative text-xl bg-gray-500/30 backdrop-blur-lg px-4 py-2 rounded-xl cursor-pointer text-slate-50" onClick={() => setEnableEdit(true)}>
